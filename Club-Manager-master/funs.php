...
function add_member($role)
{
	...
	if (isset($_POST['add_member'])) 
	{
		...
		$query = "INSERT into userinfo (name,  email, dob, username, password, role, pic) VALUES ('$name',  '$email', '$dob', '$username', '$password', '$select_role', '$pic')";
		$result = mysqli_query($con,$query);
		$rows = mysqli_affected_rows($con);
		if($rows == 1)
		{
...
function edit_member($role,$mem_id)
{
	...
	if (isset($_POST['edit_member']))
	{
		...
		{
			$query = "UPDATE userinfo SET name='$edit_name', email='$edit_email', dob='$edit_dob', username='$edit_username' WHERE id='$mem_id'";
		}
		else
		{
			$query = "UPDATE userinfo SET name='$edit_name', email='$edit_email', dob='$edit_dob' username='$edit_username', role='$edit_select_role' WHERE id='$mem_id'";
		}
